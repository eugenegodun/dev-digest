...
export const RunStats = z.object({
  ...
  // Cost in USD, computed on read (tokens × model price). Null when the model
  // is unpriced/unknown — the UI renders "—", never "$0.00".
...
export const RunSummary = z.object({
  ...
  // Cost in USD, computed on read (tokens × model price). Null when unpriced.
  cost_usd: z.number().nullable(),
