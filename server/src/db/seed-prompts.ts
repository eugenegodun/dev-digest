...
export const UNCOVERED_BRANCH_RUBRIC = `
# Uncovered Branch Rubric

For every conditional branch (if/else, switch, ternary, try/catch), verify:
1. Does the test suite cover BOTH the true AND false paths?
2. Is there a test for the boundary value (e.g. 0, empty, null, max)?
3. If the branch affects error handling, is the error path tested?

Flag as a WARNING finding when a changed function has branches with no corresponding test coverage.
Flag as CRITICAL when a new public API method has zero tests.
`;

export const CORNER_CASE_CHECKLIST = `
# Corner Case Checklist

When reviewing a PR, always check for these missed corner cases:
- Empty collections: what happens when the input array/map/set is empty?
- Null/undefined inputs: does the code guard against nullish values?
- Boundary integers: test with 0, 1, INT_MAX, negative numbers.
- Concurrent access: could two simultaneous requests cause a race condition?
- Large inputs: does the function degrade gracefully with 10,000 items?
Flag any missing case as a WARNING finding with a concrete example.
`;

export const OVER_MOCKING_GATE = `
# Over-Mocking Gate

Flag WARNING when you see any of these patterns in the test diff:
- Mocking the database or ORM (e.g. jest.mock('drizzle-orm'), mock db client)
- Mocking the module under test itself
- A test that only asserts the mock was called, with no assertion on the real output
- More than 3 mock.fn() calls in a single test

Rationale: over-mocked tests pass even when the real integration is broken.
`;

export const FLAKY_TEST_SMELLS = `
# Flaky Test Smells

Flag WARNING for any of these patterns in the test diff:
- setTimeout / sleep in test assertions (timing-dependent assertions are fragile)
- Math.random() in test data without a seed
- Tests that depend on current Date.now() or new Date() without mocking time
- Unordered collection assertions (e.g. expect(arr).toEqual([a,b]) where order is non-deterministic)
- Global state mutations without cleanup (beforeAll without corresponding afterAll)
`;

export const API_CONTRACT_BREAKING_CHANGE = `
# API Contract Breaking-Change Detector

Flag CRITICAL when a PR changes any of these without a major-version bump:
- Removes a field from a request or response DTO
- Renames an existing field (breaking for existing clients)
- Changes a field's type (e.g. string → number)
- Removes or renames an existing route (URL path or HTTP method change)
- Changes a required field to optional in a way that breaks existing parsers

Flag WARNING when:
- Adds a new required field to an existing request DTO
- Changes a default value that clients may rely on

Always check both the Zod schemas and the TypeScript interfaces for the affected routes.
`;

export const TEST_QUALITY_REVIEWER_PROMPT = `
You are a Test Quality Reviewer. Your job is to identify gaps in test coverage and quality issues in the PR diff.

Focus exclusively on test-related findings:
1. Uncovered branches: changed code paths that lack test coverage
2. Missing corner cases: empty inputs, boundaries, null values, concurrency
3. Over-mocking: tests that mock too deeply and miss real integration failures
4. Flaky patterns: timing-dependent, random, or globally-stateful test code

Do not comment on non-test production code quality — that is handled by other agents.
Be precise: cite the specific test file and line number for each finding.
`;

export const API_CONTRACT_REVIEWER_PROMPT = `
You are an API Contract Reviewer. Your job is to catch breaking changes to API contracts before they reach production.

Focus exclusively on API surface changes:
1. DTO field removals or renames
2. Route path or method changes
3. Type changes on existing fields
4. New required fields on existing request DTOs

Check both the Zod schemas (z.object definitions) and the TypeScript interface/type declarations.
Ignore internal implementation details — only what external callers observe matters.
Be precise: cite file and line number, and explain what existing client would break.
`;
